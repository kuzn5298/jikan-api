...
describe('Anime.getExternals ', () => {
    describe('success ', () => {
        const anime = getMockAnime();
        test('Loaded data', async () => {
            const { data } = await Anime.getExternals(anime.id);
    ...
    describe('error ', () => {
        test('Nonexistent id', async () => {
            expect.assertions(1);
            try {
                await Anime.getExternals(NOT_FOUND_ANIME_ID);
            } catch (e: any) {
        ...
        test('Invalid id', async () => {
            expect.assertions(1);
            try {
                await Anime.getExternals(INVALID_ANIME_ID);
            } catch (e: any) {
                expect(e.status).toBe(NOT_FOUND_CODE);
            }
        });
