...
describe('Anime.getThemes ', () => {
    describe('success ', () => {
        const anime = getMockAnime();
        test('Loaded data', async () => {
            const { data } = await Anime.getThemes(anime.id);
            expect(data.openings.length).toBeGreaterThan(0);
            expect(data.endings.length).toBeGreaterThan(0);
        });
    });
    ...
    describe('error ', () => {
        test('Nonexistent id', async () => {
            expect.assertions(1);
            try {
                await Anime.getThemes(NOT_FOUND_ANIME_ID);
            } catch (e: any) {
        ...
        test('Invalid id', async () => {
            expect.assertions(1);
            try {
                await Anime.getThemes(INVALID_ANIME_ID);
            } catch (e: any) {
                expect(e.status).toBe(NOT_FOUND_CODE);
            }
        });
